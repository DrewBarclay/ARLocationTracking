...
Realising augemented reality as an emerging technology, we began exploring technologies that could be integrated to make practical applications.
Taking inspiration from modern military shooters we came up with the idea of location tracking integrated with augmented reality, the ability to know the positions of objects and people displayed in a convienient
heads up display was appealing to us.
We set out to make a proof of concept using the android platform as the basis for our augmented reality, arduinos and ultrawide band technologies for our location tracking. Our ranging system consists of nodes in
a network, each of which is capable of sending and recieving wireless signals which we use to determine the distances from each node.
The system uses an ultrawide band transciever, the Decawave DWM1000 integrated into an Arduino pro mini to determine the distances from node to node.
We utilize custom position calculation code and ranging data from the transcievers to determine the actual position of the node in real time.
Finally our system uses this position data and sends it to an android application to display node locations on screen. Our success in integrating location tracking with augmented reality proved our concept and can be
...
\textbf{Anchor} - a stationary point used to triangulate a user’s location\\
\textbf{Billboarding} - technique used to mak an object face the screen no matter where it is situated\\
\textbf{Graphical User Interface (GUI)} - a visual interface which allows the user to see the process’ current task \\
\textbf{Heads Up Display (HUD)} - a type of GUI which displays information to the user, done without the use of a screen\\
\textbf{Homogenous coordinates} - coordinates in space which contains an extra element w which is a scaling factor on the three elements\\
\textbf{Model Matrix (M)} - a matrix containing the coordinates in the model space\\
\textbf{Model space} - space relative to the center of the object\\
\textbf{Nodes} - points in the network which show interaction between each other\\
\textbf{Normalized device coordinates (NDC)} - coordinates transformed from a 3D space to a 2D space\\
\textbf{Open GL} - an API used to render 2D and 3D graphics\\
\textbf{Perspective projection matrix} - A matrix which accounts for how objects are far away from the camera\\
\textbf{Projection Matrix} - a matrix containing the coordinates in the projection space\\
\textbf{Projection space} - Space viewed within the view of the camera\\
\textbf{Rotation Matrix ( R)} - the matrix containing the angles of orientation of an object\\
\textbf{Tag} - a mobile point to represent the user(s)\\
\textbf{Time of flight (TOF)} - a method used to calculate range using the time it takes to receive and send a signal\\
\textbf{Transformation Matrices} - matrices that are involved in the scaling, translation (movement in the space) and rotation of an object\\
\textbf{Translation matrix (T)} - the matrix containing the movement of an object in the space\\
\textbf{Ultrawide Broadband (UWB)} - a type of radio technology\\
\textbf{View Matrix (V)} - a matrix containing the coordinates in the view space\\
\textbf{View space} - Space relative to the position to which the scene is viewed\\
\textbf{World Space} - space relative to the world in which the object resides\\
